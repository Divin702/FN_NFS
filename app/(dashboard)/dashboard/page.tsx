...
export default function DashboardPage() {
  ...
  const { data: stats, isLoading: statsLoading } = useQuery({
    queryKey: dossiersKeys.stats(),
    queryFn: dossiersApi.stats,
  });
  ...
  const { data: recentDossiers, isLoading: dossiersLoading } = useQuery({
    queryKey: [...dossiersKeys.lists(), "dashboard"],
    queryFn: () => dossiersApi.list({ limit: 6 }),
  });
  const { data: recentClients, isLoading: clientListLoad } = useQuery({
              ...
              <div className="flex items-center justify-between mb-5">
                <div>
                  ...
                  <p className="text-xs text-muted mt-0.5">
                    {statsLoading ? "Loading…" : `${dossiersTotal} total`}
                  </p>
                </div>
                <Link
